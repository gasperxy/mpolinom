...
class DSEPaginator(Paginator):
    """
    Override Django's built-in Paginator class to take in a count/total number
        of items;
    Elasticsearch provides the total as a part of the query results, so we can 
        minimize hits.
    """
    ...
    def page(self, number):
        # this is overridden to prevent any slicing of the 
        # object_list - Elasticsearch has returned the sliced data already.
...
def mpoly_query(query, lte, gte):
    ...
    q0 = Q('bool', must=[
        Q("multi_match", query = query, fields = [
                'mpolynomial^4','structure_name^3','keywords^2', 'comments',
                'references','links','author^2', 'Mid^2'
            ],fuzziness = "AUTO", 
            minimum_should_match = '90%'# 90, '85%' 
        ),
        Q('range',  nb_tokens = {'lte': lte, 'gte': gte})
    ]) 
    results0 = MpolynomDocument.search().query(q0).filter("terms", 
        status=["approved", "new_comments"])
...
def index(request):
    ...
    if q:
        if q.find("x") != -1 and q.find("y") != -1 and q.find("^"):
            ...
            number_results = response0.hits.total.value 
            paginator = Paginator(results, 10)
            # if no results, search as usual (unchaged user query)
            if number_results == 0:
                results =  MpolynomDocument.search().query(
                        "multi_match", query = q, fields = [
                            'mpolynomial^3','structure_name^3','keywords^2',
                            'comments','references','links','author^2','Mid^2'
                        ],fuzziness = "AUTO"
                    ).filter(
                        "terms", status=["approved", "new_comments"]
                    )
            ...
            # user query is word
            results =  MpolynomDocument.search().query(
                "multi_match", query = q, fields = ['mpolynomial^3',
                    'structure_name^3','keywords^2','comments','references',
                    'links','author^2','Mid^2'],
                fuzziness = "AUTO"
                ).filter("terms", 
                    status=["approved", "new_comments"])
